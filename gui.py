...
windspeed_threshold_dropdown_field = html.Div(
    className="field",
    children=[
        html.Label("Select wind speed threshold", className="label"),
        html.Div(
            className="control",
            children=[
                dcc.Dropdown(
                    ...
                    options=[
                        {"label": windspeed, "value": index}
                        for index, windspeed in luts.windspeeds.items()
                    ],
                    value=25,
                )
            ],
...
*Fixme Wording Help Needed* The chart below shows modeled output from the [Weather Research and Forecasting Model](https://www.mmm.ucar.edu/weather-research-and-forecasting-model) (WRF).  Choose the model, wind speed threshold and duration threshold to see changing trends over time.
 """,
                ...
                gcm_dropdown_field,
                windspeed_threshold_dropdown_field,
